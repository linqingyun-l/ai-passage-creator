...
public interface PromptConstant {
     ...
     * 智能体2：生成大纲
     */
    String AGENT2_OUTLINE_PROMPT = " 你是一位专业的文章策划师,擅长设计文章结构。\n" +
            "     根据以下标题,生成文章大纲:\n" +
            "     主标题：{mainTitle}\n" +
            "     副标题：{subTitle}\n" +
            "            {descriptionSection}\n" +
     ...
     * 智能体3：生成正文
     */
    String AGENT3_CONTENT_PROMPT = "你是一位资深的内容创作者,擅长撰写优质文章。\n" +
            "        \n" +
            "        根据以下大纲,创作文章正文:\n" +
            "        主标题：{mainTitle}\n" +
            "        副标题：{subTitle}\n" +
            ...
            "        6. 你生成的文章不能直接生成而是因该使用Tools到网上进行检索数据，按照这些信息进行创作\n" +
            "        \n" +
    ...
    String AGENT4_IMAGE_REQUIREMENTS_PROMPT = """
            ...
            3. **在正文中插入占位符**：使用以下两种格式
               - 普通图片占位符：{{IMAGE_PLACEHOLDER_N}}，其中 N 为配图序号（2, 3, 4...），必须独占一行
               - Icon 占位符：{{ICON_PLACEHOLDER_N}}，可以放在文字行内任意位置（用于 ICONIFY 类型）
               - **重要约束：position=1 的封面图不需要也不允许在正文中放置占位符，{{IMAGE_PLACEHOLDER_1}} 绝对不能出现在正文内容中**
            ...
            6. 对于 MINIMAX 来源: 提供详细的英文生图提示词(prompt),描述场景、风格、细节
            7. 对于 MERMAID 来源: 
            ...
            12. position=1 为封面图，仅设置 imageSource 和 prompt，placeholderId 留空，不参与正文占位符编号
     ...
     * 卡通动漫风格 Prompt 附加
     */
    String STYLE_CARTOON_PROMPT = """
        **重要：请使用卡通动漫风格进行创作**
        - 语言简单明快，使用户在卡通动漫中获得信息
        - 逻辑清晰，描述卡通动漫中的角色和场景
        - 幽默风趣，使用幽默的句子和卡通元素
        - 描述卡通动漫中的物品和效果
        - 描述卡通动漫中的游戏效果和效果效果
