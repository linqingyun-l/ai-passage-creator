...
import org.example.aipassagecreator.mapper.ArticleMapper;
...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public UserStatisticsVO getUserStatistics(Long userId) {
        // 今日创作数量
        Long todayCount = countUserArticlesByDateRange(userId, getTodayStart(), LocalDateTime.now());

        // 本周创作数量
        Long weekCount = countUserArticlesByDateRange(userId, getWeekStart(), LocalDateTime.now());

        // 本月创作数量
        Long monthCount = countUserArticlesByDateRange(userId, getMonthStart(), LocalDateTime.now());

        // 总创作数量
        Long totalCount = countUserTotalArticles(userId);

        // 成功数量
        Long successCount = countUserArticlesByStatus(userId, ArticleStatusEnum.COMPLETED.getValue());

        // 失败数量
        Long failedCount = countUserArticlesByStatus(userId, ArticleStatusEnum.FAILED.getValue());

        // 成功率
        Double successRate = 0.0;
        if (totalCount > 0) {
            successRate = (successCount.doubleValue() / totalCount.doubleValue()) * 100;
        }

        // 平均耗时
        Integer avgDurationMs = calculateUserAvgDuration(userId);

        // 每日创作数量（近7天）
        List<Map<String, Object>> dailyStats = getUserDailyStats(userId, 7);

        // 剩余配额
        Long remainingQuota = getUserRemainingQuota(userId);
     ...
     * 统计指定用户时间范围内的文章数量
     */
    private Long countUserArticlesByDateRange(Long userId, LocalDateTime start, LocalDateTime end) {
        QueryWrapper queryWrapper = QueryWrapper.create()
                .eq("userId", userId)
                .ge("createTime", start)
                .le("createTime", end);
     ...
     * 统计指定用户的总文章数量
     */
    private Long countUserTotalArticles(Long userId) {
        QueryWrapper queryWrapper = QueryWrapper.create()
                .eq("userId", userId);
        return articleMapper.selectCountByQuery(queryWrapper);
    }
     ...
     * 统计指定用户指定状态的文章数量
     */
    private Long countUserArticlesByStatus(Long userId, String status) {
        QueryWrapper queryWrapper = QueryWrapper.create()
                .eq("userId", userId)
                .eq("status", status);
        return articleMapper.selectCountByQuery(queryWrapper);
     ...
     * 计算用户的平均耗时
     */
    private Integer calculateUserAvgDuration(Long userId) {
        QueryWrapper queryWrapper = QueryWrapper.create()
                .eq("userId", userId)
                .eq("status", ArticleStatusEnum.COMPLETED.getValue())
                .isNotNull("completedTime");
        ...
        try {
            ...
            log.warn("计算用户平均耗时失败, userId={}", userId, e);
        }
     ...
     * 获取用户每日创作统计
     */
    private List<Map<String, Object>> getUserDailyStats(Long userId, int days) {
        ...
        for (int i = days - 1; i >= 0; i--) {
            ...
            Long count = articleMapper.selectCountByQuery(queryWrapper);
     ...
     * 获取用户剩余配额
     */
    private Long getUserRemainingQuota(Long userId) {
        try {
            User user = userMapper.selectOneById(userId);
            if (user != null && user.getQuota() != null) {
                return user.getQuota().longValue();
            }
        } catch (Exception e) {
